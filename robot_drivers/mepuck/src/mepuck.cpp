...
Serial epuck_ser1("/dev/rfcomm1",115200, serial::Timeout::simpleTimeout(1000));
Serial epuck_ser2("/dev/rfcomm2",115200, serial::Timeout::simpleTimeout(1000));
Serial epuck_ser3("/dev/rfcomm3",115200, serial::Timeout::simpleTimeout(1000));
Serial epuck_ser4("/dev/rfcomm4",115200, serial::Timeout::simpleTimeout(1000));
Serial epuck_ser5("/dev/rfcomm5",115200, serial::Timeout::simpleTimeout(1000));
...
void epuckMotors(){
        ...
        epuck_ser1.write(sercmd);
        epuck_ser2.write(sercmd);
        epuck_ser3.write(sercmd);
        epuck_ser4.write(sercmd);
        epuck_ser5.write(sercmd);
	}

void epuckCmd(){
		epuck_ser1.write("D,50,50\n");
...
int main(int argc, char **argv){
    ...
    cout<<"Connecting to the epuck robots..."<<endl;
	/*
	try {
        epuck_ser1.open();
    }
